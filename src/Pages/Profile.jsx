      ...
      <div className="relative w-full xl:w-1/2 xl:shadow-neon  xl:p-[35px] p-[20px] pt-[69px] xl:pt-[120px] bg-background-block">
        ...
        <div className="w-full h-full">
          <div className="grid items-center justify-center mb-[24px]">
            ...
            />
            <p className="text-text-primary text-center text-[22px] xl:text-[34px]">{user?.name || 'Unknown'}</p>
            <p className="text-text-primary text-center text-[18px] xl:text-[22px]">{user?.rank || 'Player'}</p>
          </div>

          <div className="flex flex-wrap xl:flex-row items-stretch xl:items-center justify-between xl:gap-0 mb-[25px] xl:mb-[35px]">
            <button
              onClick={() => window.open(`https://steamcommunity.com/profiles/${user.steamId64}/`, '_blank')}
              type="button"
            ...
            <button
              onClick={() => navigator.clipboard.writeText(user.steamId)}
              type="button"
              className="flex items-center justify-center gap-[8px] text-accent-primary text-[18px] cursor-pointer hover:opacity-80 transition-opacity"
            >
              <img src="/img/icons/copy.svg" alt="Copy" className="w-[15px] h-[15px] " /> Copy SteamID32
            </button>

            <button
              onClick={() => navigator.clipboard.writeText(user.steamId64)}
              type="button"
              className="flex items-center justify-center gap-[8px] text-accent-primary text-[18px] cursor-pointer xl:w-auto w-full hover:opacity-80 transition-opacity"
            >
              <img src="/img/icons/copy.svg" alt="Copy" className="w-[15px] h-[15px] " /> Copy SteamID64
            </button>
          </div>
          ...
          <ul className="grid gap-[14px] xl:gap-[19px] overflow-y-auto xl:max-h-[390px]">
            ...
            <li className="flex items-center justify-between pb-[20px] border-b border-border-primary">
              ...
              <p className="text-[14px] xl:text-[18px] font-medium w-full text-left">{user?.steamId || 'N/A'}</p>
            </li>

            <li className="flex items-center justify-between pb-[20px] border-b border-border-primary">
              <p className="text-[14px] xl:text-[18px] font-medium w-full">Rank</p>
              <p className="text-[14px] xl:text-[18px] font-medium w-full text-left">{user?.rank || 'Player'}</p>
            </li>
