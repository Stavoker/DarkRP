...
router.get('/tokens', async (req, res) => {
  try {
    ...
    // Ищем колонку для токенов/кошелька
    const tokensColumn =
      columns.find((col) =>
        ['wallet', 'tokens', 'token', 'roasts_tokens', 'roast_tokens', 'money'].includes(col.Field.toLowerCase())
      )?.Field || 'wallet';

    // Ищем колонки для идентификатора и имени игрока
    const idColumn =
      columns.find((col) => ['uid', 'id', 'steamid', 'steam_id', 'steamid64'].includes(col.Field.toLowerCase()))
        ?.Field || 'uid';
...
router.get('/events', async (req, res) => {
  try {
    ...
    const steamIdColumn =
      columns.find((col) => ['steamid', 'steam_id', 'steamid64'].includes(col.Field.toLowerCase()))?.Field || 'steamid';
    ...
    const winsColumn =
      columns.find((col) => ['prize', 'wins', 'win_count', 'total_wins'].includes(col.Field.toLowerCase()))?.Field ||
      'prize';
